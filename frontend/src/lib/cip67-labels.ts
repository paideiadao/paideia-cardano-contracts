/**
 * CIP-67 Asset Name Label utilities
 * Generates properly formatted labels with CRC-8 checksums
 */

const CRC8_TABLE = [
  ,
];

export function calculateCrc8(data: Uint8Array): number {
  let crc = 0;
  for (const byte of data) {
    crc = CRC8_TABLE[(crc ^ byte) & 0xff] & 0xff;
  }
  return crc;
}

export function fromHex(hex: string): Uint8Array {
  const bytes = new Uint8Array(hex.length / 2);
  for (let i = 0; i < hex.length; i += 2) {
    bytes[i / 2] = parseInt(hex.slice(i, i + 2), 16);
  }
  return bytes;
}

export function checksum(numHex: string): string {
  return calculateCrc8(fromHex(numHex)).toString(16).padStart(2, "0");
}

export function generateCip67Label(labelNum: number): string {
  if (labelNum < 0 || labelNum > 65535) {
    throw new Error(
      `Label ${labelNum} out of range: min label 0 - max label 65535.`
    );
  }

  const numHex = labelNum.toString(16).padStart(4, "0");
  return "0" + numHex + checksum(numHex) + "0";
}

export function validateCip67Label(label: string): {
  isValid: boolean;
  labelNum?: number;
} {
  if (label.length !== 8 || !(label[0] === "0" && label[7] === "0")) {
    return { isValid: false };
  }

  const numHex = label.slice(1, 5);
  const labelNum = parseInt(numHex, 16);
  const providedCheck = label.slice(5, 7);
  const calculatedCheck = checksum(numHex);

  if (providedCheck !== calculatedCheck) {
    return { isValid: false };
  }

  return { isValid: true, labelNum };
}

export const CIP68_LABELS = {
  REFERENCE_NFT: 100,
  NFT: 222,
  FUNGIBLE_TOKEN: 333,
  RICH_FUNGIBLE_TOKEN: 444,
  ROYALTY_NFT: 500,
} as const;
